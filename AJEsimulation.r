...
# Note that Forests predict different values depending on the platform even though the seed is the same
# https://grf-labs.github.io/grf/REFERENCE.html#forests-predict-different-values-depending-on-the-platform-even-though-the-seed-is-the-same
...
ggsave("fig4.tiff") # using tiff to avoid some errors in exporting the figure as a pdf file
...
rate
# Plot the Targeting Operator Characteristic (TOC) curve.
plot(rate, xlab="Treated fraction according to CATEs",ylab="Difference in treatment effects", main=NA)
